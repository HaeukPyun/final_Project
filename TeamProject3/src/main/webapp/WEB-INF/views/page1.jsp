...
<body>
	...
	<article id="section1">
		...
		<tr>
			...
			<td><a href="${path}/myapp/page1_view?no=${list.no}">${list.title}</a></td>
			<td>${list.content}</td>
			<td>${list.writer}</td>
			<td><fmt:formatDate value="${list.writedate}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
		</tr>
		</c:forEach>
	</tbody>
			</table>
			<br>
			<form style="display:inline-block;" action="<%=request.getContextPath() %>/board?action=search" method="POST">
